...
export const uploadFileToCloudinary = async (buffer: Buffer, fileName: string): Promise<UploadApiResponse> => {
    if (!buffer && !fileName) {
        throw new AppError(status.BAD_REQUEST, "File buffer and file name are required for uploading to cloudinary");
    }
...
export const deleteFileFromCloudinary = async (url: string) => {
    try {
        ...
        throw new AppError(status.INTERNAL_SERVER_ERROR, "Failed to delete file from cloudinary", error.message);
    }
}
